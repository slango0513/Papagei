...
namespace MiniUDP
{
    ...
    public class NetPeer
    {
        ...
        public bool IsConnected => status == NetPeerStatus.Connected;
        public bool IsOpen => status != NetPeerStatus.Closed;
        public bool IsClosed => status == NetPeerStatus.Closed;
        public bool IsClient => isClient;
        public NetTraffic Traffic => traffic;
        ...
        public SocketError SendPayload(byte[] data, ushort length)
        {
            ...
            payloadSeqOut++;
            return core.SendPayload(this, payloadSeqOut, data, length);
        }
        ...
        public bool QueueNotification(byte[] data, ushort length)
        {
            ...
            core.QueueNotification(this, data, length);
            return true;
        }
        ...
        {
            NotificationReceived?.Invoke(this, data, dataLength);
        }
        ...
        internal IEnumerable<NetEvent> Outgoing => outgoing;
        internal NetPeerStatus Status => status;
        internal bool HasNotifications => outgoing.Count > 0;
        internal ushort NotificationAck => traffic.NotificationAck;
        ...
        {
            var notificationCount = outgoing.Count;
        ...
        {
            return traffic.GetTimeSinceRecv(curTime);
        }
        ...
        {
            return traffic.GeneratePing(curTime);
        }
        ...
        {
            traffic.OnReceivePing(curTime, loss);
        }
        ...
        {
            traffic.OnReceivePong(curTime, pongSeq, drop);
        }
        ...
        {
            return traffic.OnReceivePayload(curTime, sequence);
        }
        ...
        {
            AckRequested = true;
            return traffic.OnReceiveNotification(curTime, notificationSeq);
        }
