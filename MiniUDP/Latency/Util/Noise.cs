    ...
    internal class Noise
    {
      ...
      222,114, 67, 29, 24, 72,243,141,128,195, 78, 66,215, 61,156,180,
        ...
        public float GetValue(long milliseconds, float speed)
        {
            ...
            int min = floor & HASH_MASK;
            int max = (min + 1) & HASH_MASK;

            float smoothed = Lerp(Hash[min], Hash[max], smoothT);
            return smoothed / HASH_MASK;
        }

        private void Shuffle()
        {
            Random random = new Random();
            Hash = Hash.OrderBy(x => random.Next()).ToArray();
        }
    }
}
#endif